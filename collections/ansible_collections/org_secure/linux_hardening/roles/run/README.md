Org_secure.Linux_hardening Run Role
========================
...
- name: Execute tasks on servers
  hosts: servers
  roles:
    - role: org_secure.linux_hardening.run
      run_x: 42
```
...
```yaml
- name: Initialize the run role from org_secure.linux_hardening
  hosts: servers
  gather_facts: false
  tasks:
    - name: Trigger invocation of run role
      ansible.builtin.include_role:
        name: org_secure.linux_hardening.run
      vars:
        run_x: 42
```
